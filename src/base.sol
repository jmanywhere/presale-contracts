...
contract IFOByProxy is ReentrancyGuard, Initializable {
  ...
  mapping(address => mapping(uint256 =>uint256)) public tieredDataByUser;
  mapping(uint256 => uint256) public raisedByTier; // present fundraising by tier, starting at 0
  mapping(uint256 => uint256) public tierUsage; 
  uint256 public initialContribution = 1e16;  // 0.01 eth box
  uint256 public futureTierMultiplier = 3;
  uint256 public futureTierDivisor = 2;
  uint256 public currentLayer = 1;
  uint256 public oldTierCap = 1e18; // 1eth
  uint256 public boxPerRound = 10;

  function shouldIncreaseLayer() public {
    uint256 multiPlier = oldTierCap.mul(futureTierMultiplier).div(futureTierDivisor);
    if(raisedByTier[currentLayer] == multiPlier){
        currentLayer = currentLayer.add(1);
        oldTierCap = oldTierCap.mul(futureTierMultiplier).div(futureTierDivisor);
    }
    
  }

  function raiseTier(uint256 id, uint256 amount) public {
    raisedByTier[id] = amount;
  }
  
  function shouldIncreaseLayerCalculator() public view returns(uint256) {
        uint256 multiPlier = oldTierCap.mul(futureTierMultiplier).div(futureTierDivisor);
        return multiPlier;
  }

//   function checkAndUpdateTiers() internal {
//         // Implement the logic to check if a tier is sold out
//         // and update the status of the tiers accordingly
//         // For example, if Tier 2 is sold out, set tier1ParticipantsCanClaim to false
//      if(totalAmount < tier1Cap){
//         return 0; // layer 0
//      } else {
//         // tier 1 already sold out
//         // uint256 tierSecondContribution = tier1Cap.mul(futureTierMultiplier).div(futureTierDivisor);
//         // uint256 currentTier = totalAmount / tierSecondContribution;
//      }
//     }
  ...
  uint256 public maxCap = 1000000000000000000000;    // 1000 WBNB
  ...
  // presale
  uint256 public rewardPerBNB = 350000;
  ...
  function deposit() payable public {
    ...
    require(raisedByTier[currentLayer] <= oldTierCap);
